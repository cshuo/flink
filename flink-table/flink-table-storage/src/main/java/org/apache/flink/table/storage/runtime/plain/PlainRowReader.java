...
/** */
public class PlainRowReader implements RowReader {
    ...
    public CloseableIterator<RecordAndPosition<RowData>> readRecords(
        ...
        return new CloseableIterator<RecordAndPosition<RowData>>() {
            ...
            public boolean hasNext() {
                ...
                try {
                    if (iterator.advanceNext()) {
                        duplicate = iterator.getValue().getLong(0);
                        return true;
                    }
                } catch (IOException e) {
            ...
            public RecordAndPosition<RowData> next() {
                if (hasNext()) {
                    duplicate--;
                    return new RecordAndPosition<>(iterator.getKey(), 0, cnt.getAndIncrement());
                } else {
